...
def remove_icc_profile(devicepath):
    try:
        ...
        FAlog("SatBild Error: Failed to remove ICC profile", str(e))
        raise e
...
# ------------------------------------------------------------------------------------------
    ...
    def SatBild(self):
        try:
            ...
            if menu == "eumetsat":
                url = "https://cm.meteoam.it/content/published/api/v1.1/items?channelToken=8953bc366fce42c4ab0452b7fa19c164&fields=all"
                req = Request(url, headers=HEADERS)
                resp = urlopen(req, timeout=10)
                data = resp.read().decode('utf-8')
                FAlog("SatBild: meteoam data = %s" % data)

                json_data = json.loads(data)
                url = json_data['items'][0]['links'][0]['href']
                FAlog("SatBild: meteoam url = %s" % url)

                req = Request(url, headers=HEADERS)
                resp = urlopen(req, timeout=10)
                data = resp.read().decode('utf-8')
                json_data = json.loads(data)
                url = json_data['fields']['native']['links'][0]['href']
                FAlog("SatBild: meteoam native = %s" % url)
                ...
                try:
                    ...
                    self.session.open(MessageBox, _("Errore nel caricamento dell'immagine satellitare: " + str(e)), MessageBox.TYPE_ERROR)
            else:
                devicepath = "/tmp/sat.html"
                url = "%s%s?map=%s" % (BASEURL, pathname2url(self.ort), menu)
                FAlog("SatBild: Generated URL = %s" % url)

                try:
                    # <img src="https://cache.foreca.net/i/sat/__eur__-sat-20241220060000.jpg" id="mapimg" alt="Satellite" width="450" height="300">
                    # fulltext = compile(r"'(\/\/cache-.+?)'", DOTALL)
                    fulltext = compile(r'https://cache\.foreca\.net/i/sat/.+?\.jpg', DOTALL)
                    req = Request(url, headers=HEADERS)
                    resp = urlopen(req, timeout=10)
                    html_content = resp.read().decode('utf-8') if PY3 else resp.read()

                    urls = fulltext.findall(html_content)
                    if not urls:
                    ...
                    FAlog("SatBild: Found image URLs: %s" % str(urls))

                    threads = []
                    for url in urls:
                        thread = Thread(target=self.fetch_url, args=("http:%s" % url,))
...
class View_Slideshow(Screen):
    ...
    def setPicloadConf(self):
        ...
        self.picload.setPara([self["pic"].instance.size().width(), self["pic"].instance.size().height(), sc[0], sc[1], 0, int(config.plugins.foreca.resize.value), self.bgcolor])
        self["play_icon"].hide()
        if config.plugins.foreca.infoline.value is False:
            self["file"].hide()
        self.start_decode()
